...
echo -e "${verde}
┌════════════════════════════════┐
█ ${blanco}Clonando Herramienta wortex... ${verde}█
└════════════════════════════════┘"
mkdir wortex
cd SysO-Termux
chmod 777 wortex.py
mv wortex.py $HOME/wortex
cd
echo -e "${verde}
┌═══════════════════════════════┐
...
└═════════════════════════════════════┘"
git clone https://github.com/error404-notfound/ScorpFishV2 > /dev/null 2>&1
cd ScorpFishV2
...
echo -e ${verde}"
┌════════════════════════════════════════┐
█ ${blanco}¿QUIERES INSTALAR ROUTERSPLOIT? ${rojo}(ROOT) ${verde}█
└════════════════════════════════════════┘"
SI-NO
echo -e -n "${verde}┃
└═>>> "${blanco}
read -r routersploit
[ "$routersploit" == "1" ]||[ "$routersploit" == "2" ] && break
Error
done

case $routersploit in
	1)
sleep 0.5
clear
Termux
echo -e "${verde}
┌════════════════════════════┐
█ ${blanco}Instalando routersploit... ${verde}█
└════════════════════════════┘
"${blanco}
...
└═══════════════════════┘
"${blanco}
		go get http://github.com/gophish/gophish
...
echo -e "${verde}
┌═════════════════════════┐
█ ${blanco}¿QUIERES INSTALAR BEEF? ${verde}█
└═════════════════════════┘"
SI-NO
echo -e -n "${verde}┃
└═>>> "${blanco}
...
echo "alias SysO-Update='cd && rm -rf SysO-Termux && git clone https://github.com/TermuxHacking000/SysO-Termux && cd SysO-Termux && chmod 711 SysO-Update.sh && ./SysO-Update.sh'" >> $PREFIX/etc/fish/config.fish
echo "alias SysO-Lock='clear && bash ${PREFIX}/etc/bash.bashrc'" >> $PREFIX/etc/fish/config.fish
echo -e "${verde}
┌════════════════════════┐
...
echo "alias salir='exit;exit'" >> $PREFIX/etc/bash.bashrc
echo "salir" >> $PREFIX/etc/bash.bashrc
echo "alias SysO-Update='cd && rm -rf SysO-Termux && git clone https:/
/github.com/TermuxHacking000/SysO-Termux && cd SysO-Termux && chmod 71
1 SysO-Update.sh && ./SysO-Update.sh'" >> $PREFIX/etc/fish/config.fish
echo "alias SysO-Lock='clear && bash ${PREFIX}/etc/bash.bashrc'" >> $PREFIX/etc/fish/config.fish
echo -e "${verde}
┌════════════════════════┐
█ ${blanco}INSTALACIÓN FINALIZADA ${verde}█
